...
{
    ui->comboBoxBank->clear();  // Effacer la comboBox avant de la remplir / Clear the comboBox before filling it
    ui->comboBoxBank->addItem(tr("Nouvelle banque..."));  // Ajouter l'option de création / Add creation option

    // Exécuter une requête pour récupérer les noms de banques actives / Query to retrieve active bank names
    ...
    // Ajouter chaque banque dans la comboBox avec son ID / Add each bank with its ID to the comboBox
    while (query.next()) {
