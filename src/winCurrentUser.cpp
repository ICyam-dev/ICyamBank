    ...
    ui(new Ui::winCurrentUser)
{
    ...
    // Connexions des boutons aux slots correspondants / Manually connecting buttons to respective slots
...
// Charger les données utilisateur dans les champs de saisie / Load user data into input fields
void winCurrentUser::loadCurrentUserData()
{
    int folderId = databaseManager::getCurrentFolderId();  // Obtenir l'ID du dossier actuel / Get current folder ID
    QSqlQuery query(QSqlDatabase::database("ICyamBankConnection"));
    query.prepare("SELECT first_name, last_name, phone, email, addr1, addr2, addr3 FROM user WHERE id_user = :id_user");
    ...
    if (query.exec() && query.next()) {
        ...
        // Afficher un message d'erreur en cas d'échec de chargement des données / Show error message if data loading fails
        QMessageBox::warning(this, tr("Erreur"), tr("Impossible de charger les données utilisateur."));
    }
}
...
void winCurrentUser::updateUserData()
{
    ...
    // Exécuter la requête et afficher une alerte en cas d'échec / Execute the query and show alert if it fails
    if (!query.exec()) {
